...
class MailTemplate(models.Model):
    ...
    def render_template(
        ...
        get_param('web.base.url') is called in a method which replaces local
        links (links
        ...
        if company_id:
            return super(MailTemplate, self).render_template(
        ...
        records = self.env[model].browse(
            filter(None, res_ids))  # filter to avoid browsing [None]
        for rec in records:
            ...
            res = super(MailTemplate, new_self).render_template(
                template_txt, model, rec.id, post_process=post_process)
            results[rec.id] = res
